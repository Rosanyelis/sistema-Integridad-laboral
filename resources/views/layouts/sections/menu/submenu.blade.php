...
<ul class="menu-sub">
  @if (isset($menu))
    ...
    @php
      ...
      // Check if current route starts with submenu slug (for nested routes)
      elseif (str_contains($currentRouteName, $submenu->slug) && strpos($currentRouteName, $submenu->slug) === 0) {
          $activeClass = 'active';
      }
      elseif (isset($submenu->submenu)) {
        ...
        foreach ($submenu->submenu as $subSubmenu) {
          if ($currentRouteName === $subSubmenu->slug) {
            $hasActiveSubmenu = true;
            break;
          }
          // Check if current route starts with sub-submenu slug
          if (str_contains($currentRouteName, $subSubmenu->slug) && strpos($currentRouteName, $subSubmenu->slug) === 0) {
            $hasActiveSubmenu = true;
            break;
          }
        }
